...
if data[talking].text != "" {
	if data[talking].text = "BLESS" {
		draw_rectangle(
			global.width/2-string_width(data[talking].text)/2-13-7,
		...
		draw_set_alpha(1);
		draw_sprite(s_bless, 0, global.width/2-string_width(data[talking].text)/2-7, global.height/3*2.65+1);
		draw_sprite(s_bless, 0, global.width/2+string_width(data[talking].text)/2+7, global.height/3*2.65+1);
